...
SELECT 'WONDER_BUILDING_ATLAS', 256,    'BuildingPortraits256_DLC_WonderPack.dds',  2,2
WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CBPMC_TERRACE_FARM' AND Value= 1 ) UNION ALL
SELECT 'WONDER_BUILDING_ATLAS', 128,    'BuildingPortraits128_DLC_WonderPack.dds',  2,2
WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CBPMC_TERRACE_FARM' AND Value= 1 ) UNION ALL
SELECT 'WONDER_BUILDING_ATLAS', 64,     'BuildingPortraits64_DLC_WonderPack.dds',   2,2
WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CBPMC_TERRACE_FARM' AND Value= 1 ) UNION ALL
SELECT 'WONDER_BUILDING_ATLAS', 45,     'BuildingPortraits45_DLC_WonderPack.dds',   2,2
WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CBPMC_TERRACE_FARM' AND Value= 1 );

UPDATE Buildings SET
IconAtlas = 'WONDER_BUILDING_ATLAS', PortraitIndex = 0, -- Use Ziggurat icon from Wonders of the Ancient World scenario
...
SELECT 'BUILDING_STEMPLES', 'YIELD_FAITH', 'YIELD_CULTURE', 10
WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CBPMC_TERRACE_FARM' AND Value= 1);
...
'+1 [ICON_CULTURE] Culture for every 10 [ICON_PEACE] Faith generated by the City. +1 [ICON_PEACE] Faith from Terrace Farms. +1 [ICON_PEACE] Faith for every 3 Mountains within the workable plot.[NEWLINE]
...
SET Text = 'Unique {TXT_KEY_CIV_INCA_ADJECTIVE} replacement for the {TXT_KEY_BUILDING_TEMPLE}. {TXT_KEY_BUILDING_STEMPLES_DESC} converts small portion of [ICON_PEACE] Faith into [ICON_CULTURE] Culture, and increases [ICON_PEACE] Faith and [ICON_CULTURE] Culture of a city. Provides additional [ICON_PEACE] Faith from nearby Terrace Farms and Mountains.'
