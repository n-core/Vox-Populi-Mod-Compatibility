...
SET Yield = 3
WHERE BuildingType = 'BUILDING_STEMPLES' AND YieldType = 'YIELD_CULTURE'
AND EXISTS (SELECT * FROM COMMUNITY WHERE Type='CBPMC_TERRACE_FARM' AND Value= 1 );
...
SELECT 'BUILDING_STEMPLES', 'RESOURCE_AMBER', 'YIELD_GOLD', 1 WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CBPMC_TERRACE_FARM' AND Value= 1 );
...
SET Text = 'Unique {TXT_KEY_CIV_INCA_ADJECTIVE} replacement for the {TXT_KEY_BUILDING_TEMPLE}. {TXT_KEY_BUILDING_STEMPLES_DESC} increases [ICON_PEACE] Faith and [ICON_CULTURE] Culture of a city, provides [ICON_PEACE] Faith to nearby Terrace Farms and Mountains.'
