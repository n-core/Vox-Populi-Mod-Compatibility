...
UPDATE Language_EN_US
SET Text = '+20% [ICON_PRODUCTION] Production when constructing Buildings. Grocers and Granaries in the City produce +1 [ICON_FOOD] Food. Nearby Marshes and Lakes produce +3 [ICON_PRODUCTION] Production and [ICON_GOLD] Gold. +1 [ICON_FOOD] Food to Polders.'
...
SELECT 'BUILDING_WIMMEN', 'FEATURE_MARSH', 'YIELD_PRODUCTION', 3 WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CBPMC_POLDER' AND Value= 1) UNION ALL
SELECT 'BUILDING_WIMMEN', 'FEATURE_MARSH', 'YIELD_GOLD', 3 WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CBPMC_POLDER' AND Value= 1);

INSERT INTO Building_LakePlotYieldChanges (BuildingType, YieldType, Yield)
SELECT 'BUILDING_WIMMEN', 'YIELD_PRODUCTION', 3 WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CBPMC_POLDER' AND Value= 1) UNION ALL
SELECT 'BUILDING_WIMMEN', 'YIELD_GOLD', 3 WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CBPMC_POLDER' AND Value= 1);
